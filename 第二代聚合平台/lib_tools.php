<?php
if(HF_CRON=="HF_CRON"){die('Nothing!');}
require("mail/class.phpmailer.php"); //下载的文件必须放在该文件所在目录
function checkjson($uid,$type){
$nm=HF_CRON.'/json/corn_'.$type.'_'.$uid.'.json';
$mm=HF_CRON.'/config/config_'.$type.'_'.$uid.'.php';
...
function xCopy($source,$destination,$child){//$child为是否复制子目录
if(!is_dir($source)){
...
function getcorn($ccorn){
$ssa=split('\/',$ccorn);
return $ssa[count($ssa)-1];
}
?>